...
class WP_User_Query {
		...
		if ( null === $this->results ) {
			...
			if ( false !== $cache_value ) {
				...
				if ( isset( $qv['count_total'] ) && $qv['count_total'] ) {
					 ...
					 * @param WP_User_Query $query The current WP_User_Query instance.
					 */
					apply_filters( 'found_users_query', '', $this );

					$this->total_users = (int) $wpdb->last_query_total_rows;
				}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( in_array( $orderby, array( 'login', 'nicename', 'email', 'url', 'registered' ), true ) ) {
			...
			$_orderby = pn_sql_order_by_list( "$wpdb->users.ID", wp_parse_id_list( $this->query_vars['include'] ) );
		} elseif ( 'nicename__in' === $orderby ) {
			$_orderby = pn_sql_order_by_list( 'user_nicename', $this->query_vars['nicename__in'], true );
		} elseif ( 'login__in' === $orderby ) {
			$_orderby = pn_sql_order_by_list( 'user_login', $this->query_vars['login__in'], true );
