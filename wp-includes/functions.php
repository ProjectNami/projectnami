...
/**
 * T-SQL CASE equivalent of MySQL FIELD(column, v1, v2, ...).
 *
 * First value sorts first (THEN 0). Empty $values yields 0.
 *
 * @param string $column SQL column, already qualified.
 * @param array  $values Ordered list.
 * @param bool   $quote  Quote values as strings.
 * @return string
 */
function pn_sql_order_by_list( $column, $values, $quote = false ) {
	$cases = array();
	$i     = 0;
	foreach ( (array) $values as $value ) {
		if ( $quote ) {
			$lit = "'" . esc_sql( (string) $value ) . "'";
		} else {
			$lit = (string) (int) $value;
		}
		$cases[] = "WHEN {$lit} THEN {$i}";
		++$i;
	}
	if ( empty( $cases ) ) {
		return '0';
