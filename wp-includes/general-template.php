...
function wp_get_archives( $args = '' ) {
	...
	if ( 'monthly' === $parsed_args['type'] ) {
		...
		$query   = "SELECT $week AS [week], YEAR( post_date ) AS [yr], CONVERT(char(10), MIN(post_date), 23) AS [yyyymmdd], COUNT( ID ) AS [posts] FROM $wpdb->posts $join $where GROUP BY $week, YEAR( post_date ) ORDER BY YEAR( post_date ) $order, $week $order $limit";
		$key     = md5( $query );
		$key     = "wp_get_archives:$key";
		$results = wp_cache_get_salted( $key, 'post-queries', $last_changed );
		if ( ! $results ) {
...
function get_calendar( $args = array() ) {
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		...
		$d         = ( ( $w - 1 ) * 7 ) + 6;
		$thismonth = (int) $wpdb->get_var( "SELECT DATEPART(month, DATEADD(DAY, $d, '{$thisyear}0101'))" );
	} elseif ( ! empty( $m ) ) {
		$thisyear = (int) substr( $m, 0, 4 );
		if ( strlen( $m ) < 6 ) {
			$thismonth = 1;
