...
function _wp_upgrade_revisions_of_post( $post, $revisions ) {
	...
	$now    = time();
	$result = $wpdb->query( $wpdb->prepare( "IF NOT EXISTS (SELECT * FROM [$wpdb->options] with (nolock) WHERE [option_name] = '%s') INSERT [$wpdb->options] ([option_name], [option_value], [autoload]) VALUES (%s, %s, 'off') /* LOCK */", $lock, $lock, $now ) );
